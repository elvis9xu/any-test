...
public class GeoHashUtil {
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(bitsBase32(str)));
        System.out.println(Arrays.toString(splitOddEvenBits(bitsBase32(str)).get(0)));
        System.out.println(Arrays.toString(splitOddEvenBits(bitsBase32(str)).get(1)));
        System.out.println(Arrays.toString(getBase32NearPoint(str, "ALL")));
    }
    ...
    public static String[] getBase32NearPoint(String point, String nearParam) {
        ...
        for (String param : nearParams) {
            ...
            // FIXME 越界问题
            list.add(base32Bits(mergeOddEvenBits(trimBits(bitsNumber(lng), oddEventBits.get(0).length), trimBits(bitsNumber(lat), oddEventBits.get(1).length))));
        }
    ...
    public static byte[] bitsBase32(String str) {
        ...
        for (int i = 0, len = str.length(); i < len; i++) {
            System.arraycopy(trimMulBits(bitsNumber(indexBase32(str.charAt(i))), 5), 0, bits, i * 5, 5);
        }
        return bits;
    }
